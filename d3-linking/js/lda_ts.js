...
////  Bar Chart /////////////////////////////////
    ...
    var lda =  filtered_data.map(function(d) { return +d.number_topic; })
    console.log(lda)
    ...
    var bins = d3.histogram()
        .domain(barx.domain())
        .thresholds(barx.ticks(61))
    (lda);
    //console.log(bins);

    /// match values to y position
    var bary = d3.scaleLinear()
        .domain([0, d3.max(bins, function(d) { return d.length/10; })])
        //.range([280, 10]);
    ...
    bar.append("rect")
        ...
        .on("click",function(d,i) { 
            ...
            d3.selectAll("circle").filter(function(d,i) {
                return (lda[i] >= range[0]) && (lda[i] < range[1]);
    ...
    barchartsvg.append("text")
            .attr("class", "x label")
            .text("Plot Clusters (LDA Main Topic)")
            .attr("x", 500)
            .attr("y",595)
            .attr("text-anchor", "middle")
            .attr("font-family", "sans-serif")
